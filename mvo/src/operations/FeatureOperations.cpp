...
void FeatureOperations::calcProjectionsAngleDiff(std::vector<double> &depth,
  ...
  for (auto m0 = vec0.begin(), m1 = vec1.begin(); m1 != vec1.end() && m0 != vec0.end(); m1++, m0++) {
    ...
    double anglem0 = acos((R0 * *m0).dot(-b) / (cv::norm(R0 * *m0) * cv::norm(-b)));

    double anglesum = anglem0 + anglem1;

    depth.push_back(anglesum);
...
void FeatureOperations::reconstructDepth(std::vector<double> &depth,
  ...
  cv::Matx33d R = R1.t() * R0;

  for (auto m0 = vec0.begin(), m1 = vec1.begin(); m1 != vec1.end() && m0 != vec0.end(); m1++, m0++) {
    cv::Vec3d m0N = *m0 / cv::norm(*m0);
    cv::Vec3d m1N = *m1 / cv::norm(*m1);

    cv::Matx33d C = (m0N*cv::Vec3d(0,0,1).t() - cv::Matx33d::eye()).t() * (m0N*cv::Vec3d(0,0,1).t() - cv::Matx33d::eye());

    double Z = (m1N.t()*R*C*R0.t()*b)(0)/(m1N.t()*R*C*R.t()*m1N)(0);
    depth.push_back(Z);
  }
}
